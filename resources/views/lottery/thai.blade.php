...
<div class="max-w-4xl mx-auto px-4">
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 mb-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div class="p-4 bg-gray-50 rounded-lg">
                <p class="text-sm text-gray-600 mb-2">ပထမဆု</p>
                <p class="text-2xl font-bold text-gray-700">{{ $latest_result->metadata['first_prize'] ?? '000000' }}</p>
            </div>
            <div class="p-4 bg-gray-50 rounded-lg">
                <p class="text-sm text-gray-600 mb-2">နောက်ဆုံး ၂ လုံး</p>
                <p class="text-2xl font-bold text-gray-700">{{ $latest_result->metadata['last_two'] ?? '00' }}</p>
            </div>
            <div class="p-4 bg-gray-50 rounded-lg">
                <p class="text-sm text-gray-600 mb-2">ရှေ့ ၃ လုံး</p>
                <p class="text-2xl font-bold text-gray-700">{{ $latest_result->metadata['first_three'] ?? '000' }}</p>
            </div>
            <div class="p-4 bg-gray-50 rounded-lg">
                <p class="text-sm text-gray-600 mb-2">နောက် ၃ လုံး</p>
                <p class="text-2xl font-bold text-gray-700">{{ $latest_result->metadata['last_three'] ?? '000' }}</p>
            </div>
        </div>
        <p class="text-center mt-4 text-gray-600">{{ $latest_result->drawn_at->format('Y-m-d H:i:s') }}</p>
    </div>

    <!-- Play Form -->
    <div class="bg-white rounded-lg shadow-lg p-6 mb-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-bold">ထိုင်းလော့ထရီ ထိုးရန်</h2>
        ...
        <form action="{{ route('lottery.store', ['type' => 'thai']) }}" method="POST" id="lotteryForm">
            @csrf
            
            <div class="mb-6">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="play_type">
                    ထိုးမည့်အမျိုးအစား
                </label>
                <select name="play_type" 
                        id="play_type" 
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('play_type') border-red-500 @enderror"
                        required>
                    <option value="">ရွေးချယ်ပါ</option>
                    <option value="first" {{ old('play_type') === 'first' ? 'selected' : '' }}>ပထမဆု (x1000)</option>
                    <option value="last_two" {{ old('play_type') === 'last_two' ? 'selected' : '' }}>နောက်ဆုံး ၂ လုံး (x100)</option>
                    <option value="first_three" {{ old('play_type') === 'first_three' ? 'selected' : '' }}>ရှေ့ ၃ လုံး (x500)</option>
                    <option value="last_three" {{ old('play_type') === 'last_three' ? 'selected' : '' }}>နောက် ၃ လုံး (x500)</option>
                </select>
                @error('play_type')
    ...
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <ul class="list-disc list-inside space-y-2 text-gray-700">
            <li>လစဉ် ၁ရက်နေ့ နှင့် ၁၆ရက်နေ့တွင် ပေါက်ဂဏန်းထုတ်ပါသည်</li>
            <li>First Prize ပေါက်ပါက ထိုးငွေ၏ ၁၀၀၀ ဆ ပြန်လည်ရရှိမည်</li>
            <li>Last Two Digits ပေါက်ပါက ထိုးငွေ၏ ၁၀၀ ဆ ပြန်လည်ရရှိမည်</li>
            <li>First Three Digits ပေါက်ပါက ထိုးငွေ၏ ၅၀၀ ဆ ပြန်လည်ရရှိမည်</li>
            <li>Last Three Digits ပေါက်ပါက ထိုးငွေ၏ ၅၀၀ ဆ ပြန်လည်ရရှိမည်</li>
